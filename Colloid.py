...
class Colloid:
        ...
        for i in range(self.Number):  # Self interaction of the system
            for j in range(i):
                ...
                if update_repu:  # Add the repulsive influence of the particle i of the particle j.
                    ...
                    if Compute_energy:
                        self.energy_magnetic += C.magnetic_grad_constant*((np.dot(self.NP[i].m[T_new], self.NP[j].m[T_new]))-3*np.dot(
                            self.NP[i].m[T_new], u_ij)*np.dot(self.NP[j].m[T_new], u_ij))/norme_3_ij
    ...
    def external_influence(self, X=np.array([0., 0., 0.]), t=0., H_ext_model=['']):
        if len(H_ext_model) == 0:
            ...
            fator = max(t/((Frames-1)*dt*(NumberSubSteps*2+1)*(1-H_ext_model[2]))-1/(1-H_ext_model[2])+1,0.)
...
Compute_energy = False  # Computes the energy of the system
Do_statitstics = False # Computes statistical indicators
Study_Structures = False
...
for l in range(int(4)): # alea on the initial datum
    C.VerletHeunStep(0.6, l % 2, ['alea', alea_param_tr, alea_param_r], False)
    ...
    for i in range(Statistic):
        ...
        while l < NumberStep:
            ...
            if Save_100_Images and NumberStep >= 100 and l % (NumberStep//100) == 0:
                ...
                if Compute_energy and NumberStep >= 10000 and l % (NumberStep//10000) == 0:
                    C.UpdateKinetic()
                ...
                if Study_Structures and NumberStep >= 10000 and l % (NumberStep//10000) == 0:
